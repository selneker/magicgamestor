...
import { Crown, History, Rss, ShoppingCart } from "lucide-react";
    ...
    <nav className="fixed inset-x-3 bottom-3 z-40 md:hidden" data-testid="bottom-nav">
      <ul className="glass mx-auto flex max-w-md items-center justify-around rounded-full px-2 py-2 shadow-[0_8px_32px_rgba(15,23,42,0.12)]">
        {items.map(({ to, icon: Icon, label, id, active }) => (
          <li key={id}>
            <NavLink to={to} data-testid={id} className={`flex flex-col items-center gap-0.5 rounded-full px-3 py-1.5 text-[11px] font-semibold transition-colors ${active ? "bg-primary text-white" : "text-slate-500"}`}>
