...
export function StatusBadge() {
  ...
  const { t } = useLang();
  return (
    <span data-testid="admin-status-badge" className={`inline-flex items-center gap-2 border px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] ${online ? "border-[#0A0A0A] bg-primary text-[#0A0A0A]" : "border-current text-muted-foreground"}`}>
      <span className={`h-1.5 w-1.5 ${online ? "animate-pulse bg-[#0A0A0A]" : "bg-current"}`} />
...
export function Header() {
  ...
  const link = ({ isActive }) => `relative text-xs font-black uppercase tracking-[0.12em] transition-colors ${isActive ? "text-foreground after:absolute after:-bottom-1.5 after:left-0 after:h-[3px] after:w-full after:bg-primary" : "text-muted-foreground hover:text-foreground"}`;

  return (
    <header className="glass sticky top-0 z-40 border-b border-foreground">
      <div className="mx-auto flex h-16 max-w-7xl items-center gap-1.5 px-2 sm:gap-6 sm:px-6">
        <Link to="/" data-testid="header-logo" className="shrink-0 font-display text-[13px] font-black uppercase leading-none tracking-tight text-foreground sm:text-lg">
          Magic<span className="bg-primary px-1 text-[#0A0A0A]">Game</span>Store
        </Link>
        ...
        <div className="ml-auto flex min-w-0 shrink items-center gap-0 md:ml-0 md:gap-0.5">
          ...
          {user ? (
            ...
            <Button asChild size="sm" className="rounded-none border border-foreground bg-primary px-3 text-[11px] font-black uppercase tracking-[0.14em] text-[#0A0A0A] hover:bg-foreground hover:text-background sm:px-4" data-testid="login-button">
